...
import type {APICategory, APIFunction, ControlStatement, ScriptStatement} from "@/lib/types"
            ...
            toast({
                ...
                description:
                    statement.type === "function"
                        ? `Removed ${statement.function?.name || "function"} from your script`
    ...
    const sortStatementsByType = (statements: ScriptStatement[], sortType: string) => {
        ...
        if (sortType === "alphabetical-asc") {
            sortedStatements.sort((a, b) => {
                const nameA = a.type === "function" && a.function ? a.function.name : a.controlType || ""
                const nameB = b.type === "function" && b.function ? b.function.name : b.controlType || ""
            ...
            statements.forEach((stmt) => {
                ...
                } else if (stmt.type === "function" && stmt.function?.name) {
    ...
    const generateScript = () => {
        ...
        if (outsideLoopStatements.length > 0) {
            ...
            outsideLoopStatements.forEach((stmt, index) => {
                ...
                if (stmt.type === "function" && stmt.function) {
                    ...
                    <header className="bg-game-header border-b-2 border-amber-500/50 shadow-lg">
                        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
                            <div className="flex items-center">
                                <div
                                    className="w-12 h-12 rounded-lg bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center mr-3 border-2 border-amber-300 shadow-glow">
                                    <svg
                        ...
                        {activeId && activeDragData && (
                            <div className="opacity-80 transform scale-105 rotate-1">
                                {activeDragData.type === "function" && (
                                    <FunctionCard func={activeDragData.data} onAddFunction={() => {
                                    }} isDragging={true}/>
                                )}
                                {activeDragData.type === "statement" && (
                                    <div
                                        className="bg-game-card p-3 rounded-lg border-2 border-amber-500/50 shadow-glow w-full max-w-md">
